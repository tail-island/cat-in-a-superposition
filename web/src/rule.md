...
ラウンドの開始時には、手札としてカードが10枚配られます。本ゲームのカードにはスートはなく、数字のみが記載されています。Cat in a superpositionのラウンドは、以下に示す準備フェーズとトリック・フェーズ、得点フェーズの3つで構成されています。
...
* 対戦環境は、OSがWindows 11 Professional 64bitでCPUがIntel Core i7-1165G7、メモリが32GBで、ネットワークからは遮断されています。他の参加者のプログラムも同じコンピューターで実行しますので、CPUやメモリを使用しすぎないようにしてください。また、リクエストがあればGPU（NVIDIA GeForce RTX 3090）をThunderboltで接続します（申し訳ありませんが、CUDAのバージョンはGPUを必要とする参加者が合意したバージョンに固定とさせてください）。
