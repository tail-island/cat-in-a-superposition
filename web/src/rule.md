...
Cat in a superpositionは4人で4ラウンドを実施し、各ラウンドで獲得した得点の合計で順位を決定します。ラウンドでは、プレイヤーは中央のボード上に選択したカードの色を宣言していきます。その際は、中央のボードに一度宣言されたカードと色は宣言できません。ところが、Cat in a supoerpositionのカードは番号あたり<b>5枚</b>あるのに、中央のボードの色は<b>4色</b>しかありません。よって、カードの色を宣言できなくなる可能性があります。また、各プレイヤーの前に置かれたボードには、上下左右にそのプレイヤーが宣言可能な色が表示されています。プレイヤーの行動によってはこの宣言可能な色が減るため、色を宣言できなくなる可能性は大きくなります。

ラウンドの開始時には、手札としてカードが10枚配られます。本ゲームのカードにはスートはなく、数字のみが記載されています。ラウンドは、準備フェーズ、トリック・フェーズ、得点フェーズの3つえ構成されています。
...
トリック・フェーズでは、反時計回りの順番で、各プレイヤーが持っているカードから1枚選んで色を宣言していきます。ただし、中央のボードですでに宣言されている色とカードは使用できません。また、最初にカードを出したプレイヤーが宣言した色（led colorとして画面の右上に表示されています）以外の色を宣言すると、自分の前のボードからその色が消え、以降はその色を宣言できなくなります。宣言可能な色とカードの組み合わせは画面の右下に表示されていますので、その中から一つ選択してください。
...
<small>※赤が宣言されているので、赤を出せます。</small>
...
<small>勝利数と勝利予測が同じ、かつ、paradoxを宣言していない場合は、中央のボードの縦横で隣接した数がボーナス得点になります。</small>

![ボーナス得点](./image/got-bonus-point.png)
<small>最後のトリックはparadoxで終了したので勝利数に変更はなく、ボーナス得点を得ました。スコアが、勝利数1とボーナス4を合わせた5になっています。</small>

得点計算が終わると各プレイヤーの前のscoreが更新され、次のラウンドに移ります。ラウンドで最初にカードを出すのは、ラウンド数に一致するプレイヤーです。ラウンド0ではプレイやー0が、ラウンド1ではプレイヤー1が最初にカードを出すプレイヤーになります。[課題確認](./dist/index.html)でプレイする場合、あなたはプレイヤー3になので最後のラウンドで最初にカードを出すプレイヤーになります。

以上でルールの説明は終了です。以下は蛇足なのですが、プログラミング・コンクールは業務です。<b>業務として</b>[課題確認](./dist/index.html)でCat in a superpositionを<b>プレイ</b>して、ゲームのルールを理解してみてください。
...
|Player.phase|プレイヤーのアクション選択のフェーズ。0は「不要カード選択」で1は「勝利数予測」、2は「カードの色の宣言」|
...
|Parameter.actions|プレイヤーが実行したアクション|
|Board|中央のボード|
|Board.red|赤のラインの情報。宣言済みであれば宣言したプレイヤーのインデックス、そうでなければnull|
|Board.blue|青のラインの情報。宣言済みであれば宣言したプレイヤーのインデックス、そうでなければnull|
|Board.yellow|黄のラインの情報。宣言済みであれば宣言したプレイヤーのインデックス、そうでなければnull|
