...
得点計算が終わると各プレイヤーの前のscoreが更新され、次のラウンドに移ります。ラウンドで最初にカードを出すのは、ラウンド数に一致するプレイヤーです。ラウンド0ではプレイやー0が、ラウンド1ではプレイヤー1が最初にカードを出すプレイヤーになります。[課題確認](./dist/index.html)でプレイする場合あなたはプレイヤー3なので、最後のラウンドで最初にカードを出すプレイヤーになります。
...
通信には、ゲーム開始とアクション選択、観測、ゲーム終了の4種類があります。
...
ただし、Cat in a superpositionは乱数が入るので、1ゲームだけではプレイヤーの強さは測れません。プレイヤーの強さを調べたい場合は、100ゲームをプレイする`play100`を使用してください。実行するコマンドは`npm --silent run play100 "プレイヤー0を起動するコマンド" "プレイヤー1を起動するコマンド" "プレイヤー2を起動するコマンド" "プレイヤー3を起動するコマンド" 乱数シード`です。
